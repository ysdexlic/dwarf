...
" Vim Color File
" Name:       dwarf.vim
" Maintainer: https://github.com/ysdexlic/dwarf.vim/
...
let s:group_colors = {} " Cache of default highlight group settings, for later reference via `dwarf#extend_highlight`
function! s:h(group, style, ...)
  if (a:0 > 0) " Will be true if we got here from dwarf#extend_highlight
...
" public {{{
function! dwarf#set_highlight(group, style)
  call s:h(a:group, a:style)
endfunction

function! dwarf#extend_highlight(group, style)
...
if get(g:, 'dwarf_hide_endofbuffer', 0)
...
call s:h("IncSearch", { "fg": s:yellow, "bg": s:special_grey }) " 'incsearch' highlighting; also used for the text replaced with ":s///c"
...
call s:h("Search", { "fg": s:visual_black, "bg": s:special_grey }) " Last search pattern highlighting (see 'hlsearch'). Also used for similar items that need to stand out.
